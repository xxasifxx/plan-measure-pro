...
//
// IMPORTANT: This function MUST be fed ONLY approved daily reports. In the
// running app, source rows via `loadApprovedDailyReports(projectId)` which
// queries `v_approved_pay_item_quantities` — that view filters by
// `status = 'approved'`, so unreviewed inspector quantities can never reach P6.
import type {
...
export { loadApprovedDailyReports } from './load-approved';
